...
<script type="text/javascript" src="${path}/res/js/template/person/deliverAddress.js"></script>
...
<div class="w mt ofc">
	<div class="l wl">
		...
		<div class="box bg_gray">
			...
			<li><a href="${path}/user/login/toAddress.do" title="收货地址" class="here">收货地址</a></li>
	...
	<div class="r wr profile">
		...
		<div class="confirm">
			<%--
				使用jstl函数库来判断长度
			--%>
			<input type="hidden" id="addrLength" value="${fn:length(addrs)}">
			<div class="tl"></div><div class="tr"></div>
			<div class="ofc">
                ...
                <c:forEach items="${addrs}" var="addr">
					...
					<td></td>
                        <td>${addr.shipName}</td>
                        <td>${addr.provText}&nbsp;${addr.cityText}&nbsp;${addr.disText}</td>
                        <td>${addr.addr}</td>
                        <td>${addr.zipCode}</td>
                        <td>${addr.phone}</td>
				...
				<form id="jvForm" action="${path}/user/login/saveOrUpdateAddr.do" method="post">
					...
					<li>
						...
						<select id="prov" name="province">
							...
							<c:forEach items="${areaProv}" var="prov">
                                <option value="${prov.ereaId}">${prov.ereaName}</option>
                            </c:forEach>
						</select>
					...
					<li>
						...
						<span class="bg_text">
							<input type="text" id="addr" name="addr" maxLength="32" vld="{required:true}" />
						</span>
						<span class="pos">
							<span class="tip errorTip">用户名为4-20位字母、数字或中文组成，字母区分大小写。</span>
						</span>
					</li>
					<li>
