...
<head>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $(".spec a").click(function () {
                ...
                $.ajax({
                    ...
                    success:function (responseText) {
                        ...
                        $("#skuPrice").html("￥"+josnObj.sku.skuPrice);
                        $("#marketPrice").html("￥"+josnObj.sku.marketPrice);
                        if(josnObj.sku.stockInventory == 0){
                            $("#stockStatus").html("无货");
                            $("#buyNow").hide();
                            $("#addMyCar").hide();
                        }else {
                            $("#stockStatus").html(josnObj.sku.stockInventory);
                            $("#buyNow").show();
                            $("#addMyCar").show();
                        }
            ...
            //触发第一个a链接的点击事件,触发的事件必须是已定义的事件，而且必须放在已定义的事件下面
        ...
        function addCart(){
            ...
            if(validSkuStock(skuId,quantity)){
                window.location.href = path + "/cart/addItemCar.do?skuId="+skuId+"&quantity="+quantity;
            }else {
                alert("库存不足 "+quantity+" 个");
            }
        }
        function validSkuStock(skuId,quantity) {
            //默认是有库存
            var result = true;
            $.ajax({
                url:path + "/item/getSkuById.do",
                ...
                success:function (responseText) {
                    ...
                    if(jsonObj.sku.stockInventory < quantity){
                        result = false;
                    }else {
...
<div class="w ofc mt">
    ...
    <div class="r wr">
        ...
        <div class="product">
            ...
            <div class="showPro">
                <div class="big"><a id="showImg" class="cloud-zoom" href="${request_file_path}${item.imgs}" rel="adjustX:10,adjustY:-1"><img title="optional title display" alt="" src="${request_file_path}${item.imgs}"></a></div>
                <div class="small">
                    <span class="smallL" title="向左">&nbsp</span>
                    <div class="smallBox">
                        <div class="smallList">
                            <a class="cloud-zoom-gallery here" title="red" href="${request_file_path}${item.imgs}" rel="useZoom: 'showImg', smallImage: '${request_file_path}${item.imgs}'"><img alt="thumbnail 1" src="${request_file_path}${item.imgs}"></a>
                            <a class="cloud-zoom-gallery" title="blue" href="${request_file_path}${item.imgs}" rel="useZoom: 'showImg', smallImage: '${request_file_path}${item.imgs}'"><img alt="thumbnail 2" src="${request_file_path}${item.imgs}"></a>
                            <a class="cloud-zoom-gallery" title="blue" href="${request_file_path}${item.imgs}" rel="useZoom: 'showImg', smallImage: '${request_file_path}${item.imgs}'"><img alt="thumbnail 3" src="${request_file_path}${item.imgs}"></a>
                            <a class="cloud-zoom-gallery" title="blue" href="${request_file_path}${item.imgs}" rel="useZoom: 'showImg', smallImage: '${request_file_path}${item.imgs}'"><img alt="thumbnail 4" src="${request_file_path}${item.imgs}"></a>
                            <a class="cloud-zoom-gallery" title="blue" href="${request_file_path}${item.imgs}" rel="useZoom: 'showImg', smallImage: '${request_file_path}${item.imgs}'"><img alt="thumbnail 5" src="${request_file_path}${item.imgs}"></a>
                        </div>
                    </div>
            ...
            <form method="post" action="" name="" class="infor">
                <ul class="uls form">
                    ...
                    <li><label>移 动 价：</label><span class="word"><b id="skuPrice" class="f14 red mr"></b>(市场价:<del id="marketPrice"></del>)</span></li>
                    ...
                    <li><label>库　　存：</label><span id="stockStatus" class="word">有货</span></li>
                    ...
                    <ul class="uls form">
                        <li><label>规　　格：</label><div class="pre spec">
                            <#list item.skuList as sku>
                                    ...
                                    <#else >
                                    <#--将skuId默认以","分隔的格式去掉，否则值传到后台无法用Long类型接收-->
                        ...
                        <li><label>我 要 买：</label>
                            <a href="javascript:void(0);" class="inb sub"></a>
                            <input readonly type="text" id="quantity" name="" value="1" class="num" size="3" />
                            <a href="javascript:void(0);" class="inb add"></a>
                            <em id="sub_add_msg" class="red"></em>
                        </li>
                        <li class="submit">
                            <input id="buyNow" type="button" value="" class="hand btn138x40" onclick="buy();"/>
                            <input id="addMyCar" type="button" value="" class="hand btn138x40b" onclick="addCart()"/>
                            <a href="#" title="加入收藏" class="inb fav">加入收藏</a>
                        </li>
                    </ul>
                </div>
        ...
        <div class="box bg_white ofc">
            ...
            <div id="detailTab2" style="display:none">
                ...
                <table cellspacing="0" summary="" class="tab tab7">
                    ...
                    <#list item.paraValueList as paraValue>
                        <tr>
                            <th width="15%" class="alg_r">${paraValue.featureName}</th>
                            <td>${paraValue.paraValue}</td>
                        </tr>
                    </#list>
                    </tbody>
                </table>
