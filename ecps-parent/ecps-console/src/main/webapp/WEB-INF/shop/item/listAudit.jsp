...
<script type="text/javascript" src="${path}/ecps/console/res/js/jquery.tablesorter.js"></script>
...
<form action="${path}/item/auditItem.do" method="post" id="auditForm">
	...
	<input type="hidden" id="auditStatus3" name="auditStatus">
...
<div class="frameR"><div class="content">
	...
	<form id="form1" name="form1" action="${path}/item/listAudit.do" method="post">
		<input type="hidden" id="auditStatus2" value="${qc.auditStatus}" name="auditStatus2">
		<div class="sch">
			<p>搜索：
		...
		<table cellspacing="0" summary="" class="tab" id="myTable">
			...
			<tbody>
			<c:forEach items="${page.lists}" var="item">
				<tr>
					<td>${item.itemNo}</td>
					<td >${item.itemName}</td>
					...
					<td>
						...
						<c:if test="${item.auditStatus == 2}">未通过</c:if>
					</td>
        ...
        <span class="l inb_a">
            ...
            <a href="javascript:void(0);" id="lastPage"  title="尾页">尾页</a>
        </span>
		</div>
	</form>
</div></div>
